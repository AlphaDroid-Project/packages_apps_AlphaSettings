...
public class MonetSettings extends DashboardFragment implements OnPreferenceChangeListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mThemeStyle = findPreference(PREF_THEME_STYLE);
        mColorSource = findPreference(PREF_COLOR_SOURCE);
        mAccentColor = findPreference(PREF_ACCENT_COLOR);
        mBgColor = findPreference(PREF_BG_COLOR);
        mLuminance = findPreference(PREF_LUMINANCE);
        mChroma = findPreference(PREF_CHROMA);
        mTintBackground = findPreference(PREF_TINT_BACKGROUND);
        mFidelity = findPreference(PREF_FIDELITY);
    ...
    private void updatePreferences() {
        try {
            ...
            mCurrentSource = (both && COLOR_SOURCE_HOME.equals(source)) ? "both" : source;
    ...
    private void applyBackgroundTintPolicy(String style) {
        ...
        if (disallow) {
            ...
            mTintBackground.setSummary(style.equals(STYLE_MONOCHROMATIC)
